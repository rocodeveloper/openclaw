...
describe("isBillingErrorMessage", () => {
  ...
  it("matches Google/OpenAI quota bill-outs ('check your plan and billing details')", () => {
    // Provider bill-outs arrive as HTTP 429 but say "plan and billing details".
    // Without this they were mis-classified rateLimit, so the funny billing copy
    // never fired and the user saw a raw provider error.
    const google =
  ...
  it("does NOT treat transient per-minute / resource-exhausted 429s as billing", () => {
    // These lack "plan and billing" and must stay retryable (rate_limit).
    expect(
      isBillingErrorMessage(
